...
export default async function AdminVoucherDetailPage({
  ...
  const userIds: string[] = Array.from(new Set(voucher.redemptions.map((redemption: any) => String(redemption.userId))));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="card">
          ...
          <p className="text-lg font-semibold mt-1">
            {voucher.expiresAt ? new Date(voucher.expiresAt).toLocaleString() : 'Sin fecha límite'}
          </p>
        </div>
      </div>
      ...
      <div className="card space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Pedido mínimo: {voucher.minOrderValue ? `€ ${voucher.minOrderValue.toFixed(2)}` : 'No definido'}
        </p>
      </div>
      ...
      <div className="card">
        ...
        {voucher.redemptions.length === 0 ? (
          <p className="text-sm text-muted-foreground">Aún no hay usos registrados.</p>
        ) : (
                ...
                {voucher.redemptions.map((redemption: any) => {
                  ...
                  return (
                    <tr key={redemption.id} className="border-t border-border">
                      <td className="px-3 py-2 text-sm">{new Date(redemption.redeemedAt).toLocaleString()}</td>
                      <td className="px-3 py-2 text-sm">{user?.name || 'Sin nombre'}</td>
                      <td className="px-3 py-2 text-sm">{user?.email || redemption.userId}</td>
                      <td className="px-3 py-2 text-xs font-mono">{redemption.purchase?.id || '-'}</td>
                      <td className="px-3 py-2 text-sm">
                        {redemption.purchase?.originalPrice !== null && redemption.purchase?.originalPrice !== undefined
                          ? `€ ${redemption.purchase.originalPrice.toFixed(2)}`
                          : '-'}
                      </td>
                      <td className="px-3 py-2 text-sm">
                        {redemption.purchase?.discountAmount !== null && redemption.purchase?.discountAmount !== undefined
                          ? `€ ${redemption.purchase.discountAmount.toFixed(2)}`
                          : '-'}
                      </td>
                      <td className="px-3 py-2 text-sm">
                        {redemption.purchase?.finalPrice !== null && redemption.purchase?.finalPrice !== undefined
