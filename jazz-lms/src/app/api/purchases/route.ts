...
import { listRecentLemonOrdersByEmail, retrieveLemonOrder } from '@/lib/lemon-squeezy';
...
function isLemonApiError(error: unknown): boolean {
  ...
  return message.includes('Lemon API failed') || message.includes('Lemon checkout create failed');
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!process.env.LEMON_SQUEEZY_API_KEY?.trim()) {
      return NextResponse.json({ purchased: false, reason: 'payments_unavailable' }, { status: 503 });
    }
    ...
    if (!orderId) {
      ...
      if (!currentUserEmail) {
        return NextResponse.json({ purchased: false, reason: 'pending_webhook' }, { status: 202 });
      }

      let recentOrders: Awaited<ReturnType<typeof listRecentLemonOrdersByEmail>> = [];
      ...
      const candidates = recentOrders.filter((order) => {
        ...
        const customCourseId = readCustomString(customData, 'courseId', 'course_id');
        ...
        if (checkoutAttemptId) {
          ...
          if (!customCheckoutAttemptId || customCheckoutAttemptId !== checkoutAttemptId) {
            return false;
          }
        }
      ...
      if (!selectedOrder && checkoutAttemptId && candidates.length > 1) {
        selectedOrder = [...candidates].sort((left, right) => {
          const leftEpoch = Date.parse(asString(asObject(left.attributes).created_at) ?? '') || 0;
          const rightEpoch = Date.parse(asString(asObject(right.attributes).created_at) ?? '') || 0;
      ...
      const totalAmount =
        toMoney(selectedAttributes.total) || toMoney(selectedAttributes.total_formatted) || subtotalAmount;
      const discountAmount = Number(Math.max(0, subtotalAmount - totalAmount).toFixed(2));
      const voucherCode = readCustomString(selectedCustomData, 'voucherCode', 'voucher_code');
      const providerDiscountCode =
        asString(selectedAttributes.discount_code) ??
        readCustomString(selectedCustomData, 'providerDiscountCode', 'provider_discount_code');
      ...
      try {
        ...
        if (isPrismaSchemaMismatchError(error)) {
          ...
          return NextResponse.json({ purchased: false, reason: 'database_schema_mismatch' }, { status: 503 });
        }
    ...
    try {
      ...
      if (isLemonApiError(error)) {
        ...
        return NextResponse.json({ purchased: false, reason: 'provider_unavailable' }, { status: 503 });
      }
    ...
    if (!lemonOrder) {
      return NextResponse.json({ purchased: false, reason: 'order_not_found' }, { status: 404 });
    }
    ...
    if (orderStatus !== 'paid') {
      return NextResponse.json({ purchased: false, reason: 'order_not_paid' }, { status: 409 });
    }

    const orderUserEmail = asString(attributes.user_email)?.toLowerCase();
    const currentUserEmail = user.email?.trim().toLowerCase() ?? null;

    if (!orderUserEmail || !currentUserEmail || orderUserEmail !== currentUserEmail) {
      return NextResponse.json({ purchased: false, reason: 'order_user_mismatch' }, { status: 403 });
    }
    ...
    if (customCourseId && customCourseId !== courseId) {
      return NextResponse.json({ purchased: false, reason: 'order_course_mismatch' }, { status: 400 });
    }

    if (customUserId && customUserId !== user.id) {
      return NextResponse.json({ purchased: false, reason: 'order_user_metadata_mismatch' }, { status: 403 });
    }
