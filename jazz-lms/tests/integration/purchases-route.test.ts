...
describe("GET /api/purchases", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.getPaymentProvider.mockReturnValue("lemon");
...
describe("POST /api/purchases", () => {
  ...
  test("returns 202/pending_webhook immediately when dodo provider is active", async () => {
    ...
    expect(mocks.listRecentLemonOrdersByEmail).not.toHaveBeenCalled();
    expect(mocks.retrieveLemonOrder).not.toHaveBeenCalled();
  });
  ...
  test("reconciles dodo purchase from processed webhook events", async () => {
    ...
    expect(mocks.listRecentLemonOrdersByEmail).not.toHaveBeenCalled();
    expect(mocks.retrieveLemonOrder).not.toHaveBeenCalled();
    expect(mocks.retrieveDodoPayment).not.toHaveBeenCalled();
  });

  test("reconciles dodo purchase from provider API when webhook event is not available", async () => {
    mocks.getPaymentProvider.mockReturnValue("dodo");
    mocks.getUser.mockResolvedValue({
      data: { user: { id: "u1", email: "admin@neurofactory.net" } },
    });
    ...
    mocks.retrieveDodoPayment.mockResolvedValue({
      ...
      metadata: {
        userId: "u1",
        courseId: "course-1",
        checkoutAttemptId: "attempt-api-1",
      },
    });
    ...
    expect(mocks.retrieveDodoPayment).toHaveBeenCalledWith("pay_api_1");
    expect(mocks.upsertCoursePurchaseFromProvider).toHaveBeenCalledWith(
  ...
  test("returns 503/provider_unavailable when dodo payment retrieval fails", async () => {
    mocks.getPaymentProvider.mockReturnValue("dodo");
    mocks.getUser.mockResolvedValue({
      data: { user: { id: "u1", email: "admin@neurofactory.net" } },
    });
    ...
    mocks.retrieveDodoPayment.mockRejectedValue(
      new Error("Dodo payment retrieve failed (503): unavailable"),
    );
  ...
  test("returns 202/pending_webhook when recent orders lookup fails in provider", async () => {
    ...
    mocks.purchaseFindUnique.mockResolvedValue(null);
    mocks.listRecentLemonOrdersByEmail.mockRejectedValue(
      new Error("Lemon API failed (403): forbidden"),
    );
  ...
  test("returns 503/provider_unavailable when retrieve order fails in provider", async () => {
    ...
    mocks.purchaseFindUnique.mockResolvedValue(null);
    mocks.retrieveLemonOrder.mockRejectedValue(
      new Error("Lemon API failed (401): unauthorized"),
    );
  ...
  test("returns 503/database_schema_mismatch when purchase upsert fails with missing table/column", async () => {
    ...
    const prismaStyleError = Object.assign(
      new Error("The table `DiscountApplied` does not exist"),
      {
        code: "P2021",
      },
    );
